...
public class DefaultListableBeanFactoryTests {
	...
	/**
	 * #RoboSpring: Lowered the number of iterations from 1000 to 10 to prevent a StackOverFlowException
	 */
	@Test
	public void testExtensiveCircularReference() {
		...
		for (int i = 0; i < 10; i++) {
			MutablePropertyValues pvs = new MutablePropertyValues();
			pvs.addPropertyValue(new PropertyValue("spouse", new RuntimeBeanReference("bean" + (i < 9 ? i + 1 : 0))));
			RootBeanDefinition bd = new RootBeanDefinition(TestBean.class, pvs);
		...
		for (int i = 0; i < 10; i++) {
			...
			TestBean otherBean = (TestBean) lbf.getBean("bean" + (i < 9 ? i + 1 : 0));
	...
	@Test
	@Ignore("RoboSpring: Performance test ignored for mobile")
