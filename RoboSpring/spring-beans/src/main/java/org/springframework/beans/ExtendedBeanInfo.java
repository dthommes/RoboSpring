	...
	public ExtendedBeanInfo(BeanInfo delegate) throws IntrospectionException {
		...
		for (MethodDescriptor md : delegate.getMethodDescriptors()) {
			...
			for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
				...
				Method pdReadMethod = pd.getReadMethod();
				for (PropertyDescriptor existingPD : this.propertyDescriptors) {
					if (method.equals(pdReadMethod)
				...
				if (method.equals(pdReadMethod)
					...
					if (pd instanceof IndexedPropertyDescriptor) {
						this.addOrUpdatePropertyDescriptor(pd, pd.getName(), pdReadMethod, pd.getWriteMethod(), ((IndexedPropertyDescriptor)pd).getIndexedReadMethod(), ((IndexedPropertyDescriptor)pd).getIndexedWriteMethod());
					} else {
						this.addOrUpdatePropertyDescriptor(pd, pd.getName(), pdReadMethod, pd.getWriteMethod());
					}
					continue ALL_METHODS;
				}
			}
